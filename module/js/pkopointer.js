...
var Pointer = function Pointer(scene) {
	this.scene = scene;
	this.active = false;
...
Pointer.prototype.update = function update() {
...
var GestureTracker = function GestureTracker(scene) {
	var self = this;
	...
	this.handleMouseMove = function handleMouseMove(e) {
		e.preventDefault();
	...
	this.handleMouseUp = function handleMouseUp(e) {
		e.preventDefault();
		if (e.which == 1) {
	...
	this.getCurrentTouchIndex = function getCurrentTouchIndex(idToFind) {
	...
	this.handleTouchStart = function handleTouchStart(e) {
		e.preventDefault();
	...
	this.handleTouchMove = function handleTouchMove(e) {
		e.preventDefault();
	...
	this.handleTouchEnd = function handleTouchEnd(e) {
		e.preventDefault();
		var touches = e.changedTouches;
		var index;
...
GestureTracker.prototype.update = function update() {
	for (var i=0; i<this.pointer.length; i++) {
		this.pointer[i].update();
	}
}

GestureTracker.prototype.initMouse = function initMouse() {
